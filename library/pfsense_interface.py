...
class pfSenseInterface(object):
    ...
    def check_infos(self, ip, mask, gateway, interface):
        ...
        rcode = call("ifconfig |grep '^"+interface+"' 1>/dev/null 2>&1", shell=True)
        if rcode != 0:
            self.module.fail_json(msg="Interface %s not found on pfsense" % (interface))
    ...
    def remove(self, interface):
        ...
        if interfaceEl is not None:
            if interfaceEl.tag == "lan" or interfaceEl.tag == "wan":
                self.module.fail_json(msg='Delete lan or wan interface unauthorized with this module')
            if self.module.check_mode:
                self.module.exit_json(changed=True)
...
def main():
    ...
    interface['spoofmac'] = ''
    if state == 'absent':
        pfinterface.check_infos(interface['ipaddr'], interface['subnet'], interface['gateway'], interface['if'])
